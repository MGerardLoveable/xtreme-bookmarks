// Lightweight Lucide SVG icon set — only the icons we use.
// Bundled inline to keep the app offline-friendly (no CDN dependency).
...
const ICONS = {
  ...
  'download-cloud': '<path d="M12 13v8l-4-4"/><path d="m12 21 4-4"/><path d="M4.393 15.269A7 7 0 1 1 15.71 8.05"/>',
  'rows-3': '<rect x="3" y="3" width="18" height="5" rx="1"/><rect x="3" y="11" width="18" height="5" rx="1"/><rect x="3" y="19" width="18" height="2" rx="1"/>',
  ...
  'radar': '<path d="M19.07 4.93A10 10 0 0 1 4.93 19.07"/><path d="M14.12 9.88a3 3 0 1 1-4.24 4.24"/><path d="M8.5 8.5a5 5 0 0 1 7 7"/><path d="M12 12h.01"/><path d="M2 2l20 20"/>',
  ...
  'github': '<path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.4 5.4 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65S9 17.38 9 18v4"/><path d="M9 18c-4.51 2-5-2-7-2"/>',
  ...
  'database-backup': '<ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v7c0 1.66 4.03 3 9 3 .86 0 1.69-.04 2.45-.12"/><path d="M3 12v7c0 1.66 4.03 3 9 3 1.21 0 2.37-.08 3.38-.24"/><path d="M21 5v6"/><path d="m16 19 2 2 4-4"/>',
...
function buildSvg(name) {
  ...
  if (!body) return null;
  return `<svg xmlns="${SVG_NS}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">${body}</svg>`;
}

/** Renders all [data-icon] spans inside a root element. */
export function renderIcons(root = document) {
  const nodes = root.querySelectorAll('[data-icon]');
  nodes.forEach((node) => {
    if (node.dataset.iconRendered === '1') return;
...
/** Returns an inline string for dynamic HTML templates. */
