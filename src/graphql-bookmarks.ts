...
function isUsableSessionCache(
  ...
  browserId: string,
  profileDirectory?: string,
): cache is CachedSessionCookies {
  return Boolean(
    cache &&
    cache.browserId === browserId &&
    cache.profileDirectory === profileDirectory &&
    cache.csrfToken &&
    cache.cookieHeader &&
    Date.now() - cache.savedAt < SESSION_COOKIE_CACHE_TTL_MS
  );
}

async function loadCachedSessionCookies(
  browserId: string,
  profileDirectory?: string,
): Promise<CachedSessionCookies | null> {
  if (isUsableSessionCache(cachedSessionCookies, browserId, profileDirectory)) {
  ...
  try {
    const cache = await readJson<CachedSessionCookies>(cachePath);
    if (!isUsableSessionCache(cache, browserId, profileDirectory)) return null;
...
async function loadAnyCachedSessionCookies(): Promise<CachedSessionCookies | null> {
  if (
    cachedSessionCookies &&
    cachedSessionCookies.csrfToken &&
    cachedSessionCookies.cookieHeader &&
    Date.now() - cachedSessionCookies.savedAt < SESSION_COOKIE_CACHE_TTL_MS
  ) {
    return cachedSessionCookies;
  }
  ...
  try {
    const cache = await readJson<CachedSessionCookies>(cachePath);
    if (!cache.csrfToken || !cache.cookieHeader || Date.now() - cache.savedAt >= SESSION_COOKIE_CACHE_TTL_MS) return null;
    cachedSessionCookies = cache;
    return cache;
  } catch {
    return null;
...
async function saveCachedSessionCookies(cache: CachedSessionCookies): Promise<void> {
  cachedSessionCookies = cache;
  ensureDataDir();
...
export function buildXGraphQLHeaders(csrfToken: string, cookieHeader?: string): Record<string, string> {
  return {
...
export async function loadXGraphQLSession(
  options: Pick<SyncOptions, 'browser' | 'chromeUserDataDir' | 'chromeProfileDirectory' | 'firefoxProfileDir' | 'csrfToken' | 'cookieHeader'> = {},
  ...
  try {
    ...
    const session = {
      browserId: config.browser.id,
      profileDirectory: chromeProfile,
      csrfToken,
      cookieHeader: cookieHeader ?? `ct0=${csrfToken}`,
      savedAt: Date.now(),
    };
    await saveCachedSessionCookies(session);
    return { csrfToken: session.csrfToken, cookieHeader: session.cookieHeader };
  } catch (err) {
    if (!cached) throw err;
    return { csrfToken: cached.csrfToken, cookieHeader: cached.cookieHeader };
  }
}
