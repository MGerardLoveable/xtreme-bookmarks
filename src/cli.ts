...
import { getGraphStats, exportGraphAsMermaid } from './graph.js';
...
function logo(): string {
  ...
  const vLabel = `v${v}`;
  const innerW = 37;
  const line1 = 'X t r e m e   B o o k m a r k s';
  const line2 = '2nd Brain · Knowledge Engine';
  const pad1 = Math.max(innerW - line1.length - 3, 1);
  const pad2 = Math.max(innerW - line2.length - vLabel.length - 4, 1);
  return `
     \x1b[2m\u250c${'\u2500'.repeat(innerW)}\u2510\x1b[0m
     \x1b[2m\u2502\x1b[0m  \x1b[1;36m${line1}\x1b[0m${' '.repeat(pad1)} \x1b[2m\u2502\x1b[0m
     \x1b[2m\u2502\x1b[0m  \x1b[2m${line2}\x1b[0m${' '.repeat(pad2)}\x1b[2m${vLabel}\x1b[0m  \x1b[2m\u2502\x1b[0m
     \x1b[2m\u2514${'\u2500'.repeat(innerW)}\u2518\x1b[0m`;
}
...
export function showWelcome(): void {
  console.log(logo());
  console.log(`
  Your X/Twitter bookmarks as a persistent knowledge base.
  Sync, classify, build a wiki, and run maintenance agents.
  ...
  Works with Chrome, Brave, Chromium, and Firefox.
...
export function buildCli() {
  ...
  async function rebuildIndex(): Promise<number> {
    ...
    process.stderr.write(`  \u2713 ${result.recordCount} bookmarks indexed (${result.newRecords} new), categories assigned\n`);
  ...
  program
    .name('xb')
    .description('Xtreme Bookmarks — 2nd Brain knowledge engine for your X/Twitter bookmarks.')
    .version('2.0.0')
    .showHelpAfterError()
    .hook('preAction', () => {
      console.log(logo());
...
`);
  ...
  program
    .command('web')
    .description('Open a local web UI for browsing and searching bookmarks')
    .option('--port <n>', 'Port to listen on', (v: string) => Number(v), 3848)
    .action(safe(async (options) => {
      if (!requireIndex()) return;
      await startWebServer(Number(options.port) || 3848);
    }));
  ...
  program
    ...
    .argument('[engine]', 'Set default engine directly (e.g. grok, grok-api, claude, codex)')
    .action(safe(async (engineArg?: string) => {
      const available = detectAvailableEngines();
      const prefs = loadPreferences();

      if (available.length === 0) {
        console.log('  No LLM engines are ready yet.');
        console.log('  Set up one of:');
        console.log('    - SuperGrok OAuth: install the xAI Grok CLI, then run: grok');
        console.log('    - Grok API:         set XAI_API_KEY in .env.local or your shell');
      ...
      if (engineArg) {
        ...
        engineArg = aliases[engineArg] ?? engineArg;
  ...
  program
    .command('auth')
    .description('Set up OAuth for API sync and X bookmark delete/write actions')
    .action(safe(async () => {
  ...
  program
    .command('wiki')
    .description('Compile Karpathy-style markdown wiki from curated bookmarks (marked via Web UI)')
  ...
  // ── consolidate ── Tiered memory sweep ──────────────────────────

  program
    .command('consolidate')
    .description('Run memory consolidation: Working → Episodic → Semantic')
    .option('--json', 'Output JSON')
    .action(safe(async (options) => {
      if (!requireIndex()) return;
      console.log('\n  Running memory consolidation sweep...\n');
      const result = await consolidateMemoryTiers(undefined, (s) => console.log(s));
      ...
      const tiers = await getMemoryTierStats();
      console.log('\n  Memory Tier Distribution:');
      console.log(`    Working:    ${tiers.working}`);
      console.log(`    Episodic:   ${tiers.episodic}`);
      console.log(`    Semantic:   ${tiers.semantic}`);
      console.log(`    Procedural: ${tiers.procedural}`);
      console.log();
    }));
  ...
  program
    .command('graph')
    .description('Show knowledge graph stats and optionally export as Mermaid diagram')
    .option('--mermaid', 'Export graph as Mermaid diagram')
    .option('--json', 'Output JSON stats')
    .action(safe(async (options) => {
      const stats = await getGraphStats();

      if (options.mermaid) {
        const mermaid = await exportGraphAsMermaid();
        console.log(mermaid);
        return;
      }
      ...
      if (stats.topConnected.length > 0) {
        console.log('\n  Top Connected:');
        for (const tc of stats.topConnected.slice(0, 5)) {
          console.log(`    ${tc.id} (${tc.connections} connections)`);
        }
      }
      console.log();
    }));

  // ── health ── Full system health report (OpenSpec) ──────────────────

  program
    .command('health')
    .description('Run all maintenance agents and produce a health report')
    .option('--llm', 'Use LLM for AI-powered contradiction resolution')
    .option('--json', 'Output JSON (OpenSpec format)')
    .action(safe(async (options) => {
      const report = await runMaintenanceAgent(
        (s) => console.log(s),
  ...
  const brain = program
    .command('brain')
    .description('Manage Main Brain, Sub-Brains, and research agents');

  brain
    .command('list')
    .description('List Sub-Brain spaces')
    .action(safe(async () => {
      if (!requireIndex()) return;
      const spaces = await listBrainSpaces();
      if (spaces.length === 0) {
        console.log('  No Sub-Brains yet. Create one with: xb brain create "AI Research" --keywords ai,research --repo karpathy/autoresearch');
        return;
      }
      for (const s of spaces) {
        console.log(`  ${s.id.padEnd(24)} ${String(s.bookmarkCount).padStart(4)} bookmarks  ${String(s.repoCount).padStart(2)} repos  ${s.name}`);
        if (s.keywords.length) console.log(`    keywords: ${s.keywords.join(', ')}`);
        if (s.lastAgentRunAt) console.log(`    last agents: ${s.lastAgentRunAt}`);
      }
    }));

  brain
    .command('create')
    .description('Create a Sub-Brain topic space')
    .argument('<name>', 'Sub-Brain name')
    .option('--description <text>', 'Description')
    .option('--keywords <csv>', 'Comma-separated seed keywords')
    .option('--category <slug>', 'Seed from bookmark category')
    .option('--domain <slug>', 'Seed from subject domain')
    .option('--collection <name>', 'Seed from collection')
    .option('--repo <owner/name...>', 'GitHub repos to watch')
    .action(safe(async (name: string, options) => {
      if (!requireIndex()) return;
      const space = await createBrainSpace({
        name,
        description: options.description,
        keywords: options.keywords ? String(options.keywords).split(',') : [],
        category: options.category,
        domain: options.domain,
        collection: options.collection,
      });
      for (const repo of options.repo ?? []) {
        await addBrainRepo(space.id, { repo, source: 'cli' });
      }
      const seeded = await seedBrainSpace(space.id);
      console.log(`  Created ${space.name} (${space.id})`);
      console.log(`  Seeded ${seeded.added}/${seeded.matched} matching bookmarks`);
      if ((options.repo ?? []).length > 0) console.log(`  Watching repos: ${(options.repo ?? []).join(', ')}`);
    }));

  brain
    .command('seed')
    .description('Seed a Sub-Brain from its hybrid rules')
    .argument('<space>', 'Sub-Brain id or name')
    .action(safe(async (space: string) => {
      if (!requireIndex()) return;
      const result = await seedBrainSpace(space);
      console.log(`  ${result.space.name}: ${result.added}/${result.matched} matching bookmarks added`);
    }));

  brain
    .command('run')
    .description('Run repo watcher and research scout agents')
    .argument('<space>', 'Sub-Brain id/name or "all"')
    .action(safe(async (space: string) => {
      if (!requireIndex()) return;
      const result = await runBrainAgents(space);
      console.log(`  Ran agents for ${result.spaces} Sub-Brain(s); ${result.findings} finding(s).`);
    }));

  brain
    .command('repos')
    .description('List or add watched GitHub repos for a Sub-Brain')
    .argument('<space>', 'Sub-Brain id or name')
    .argument('[repos...]', 'Repos to add, e.g. karpathy/autoresearch')
    .action(safe(async (space: string, repos: string[]) => {
      if (!requireIndex()) return;
      if (repos.length > 0) {
        for (const repo of repos) await addBrainRepo(space, { repo, source: 'cli' });
      }
      const watched = await listBrainRepos(space);
      if (watched.length === 0) {
        console.log('  No repos watched yet.');
        return;
      }
      for (const repo of watched) {
        console.log(`  ${repo.repo}${repo.lastCheckedAt ? `  checked ${repo.lastCheckedAt}` : ''}`);
      }
    }));

  // ── lint ── Health-check the markdown wiki ─────────────────────────
