...
export function dataDir(): string {
  const override = process.env.XTREME_BOOKMARKS_DATA_DIR;
  if (override && override !== 'undefined') return path.resolve(override);
  const legacyOverride = process.env.FT_DATA_DIR;
  if (legacyOverride && legacyOverride !== 'undefined') return path.resolve(legacyOverride);
  return path.join(os.homedir(), '.xtreme-bookmarks');
}
...
export function browserSessionCachePath(): string {
  return path.join(dataDir(), 'browser-session-cache.json');
}
...
export function mdBookmarksDir(): string {
