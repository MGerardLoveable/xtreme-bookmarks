...
async function doCompile(
  ...
  try {
    ...
    progress('Scanning curated bookmarks...');
    const categoryCounts = await getCategoryCounts(db, true);
    const domainCounts   = await getDomainCounts(db, true);
    const topAuthors     = await getTopAuthorHandles(100, db, true);
    ...
    for (let i = 0; i < toGenerate.length; i++) {
      ...
      if (item.type === 'category') {
        samples = await sampleByCategory(item.name, MAX_SAMPLE_SIZE, db, true);
        prompt  = buildCategoryPagePrompt(item.name, mapToMdBookmarks(samples));
      } else if (item.type === 'domain') {
        samples = await sampleByDomain(item.name, MAX_SAMPLE_SIZE, db, true);
        prompt  = buildDomainPagePrompt(item.name, mapToMdBookmarks(samples));
      } else {
        samples = await sampleByAuthor(item.name, MAX_SAMPLE_SIZE, db, true);
...
export async function addBookmarkToWiki(id: string): Promise<{ created: boolean; updated: boolean; path: string }> {
  return { created: false, updated: false, path: '' };
}
